...
const WeddingColourButtons = () => {
  ...
  return (
    <div className="flex gap-1 sm:gap-3  md:gap-5 self-center items-center justify-center m-5 ">
      {colours.map((color, i)=>{
        return <p key={color} className={`w-20 h-20 rounded-full border-[var(--background)] border-[2px]  ${colours[i]}`} /* onMouseEnter={()=>{setShowText(true)}} onMouseLeave={()=>{setShowText(false)}}*/>
        </p>
      })}
    </div>
  )
